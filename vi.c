...
// #define CONFIG_FEATURE_VI_CRASHME		// randomly pick commands to
// execute
...
#if ENABLE_FEATURE_VI_8BIT
#define Isprint(c)                                                             \
  ((unsigned char)(c) >= ' ' && (c) != 0x7f && (unsigned char)(c) != 0x9b)
#else
...
static void sync_cursor(char *, int *,
                        int *);         // synchronize the screen cursor to dot
...
static char *text_hole_delete(char *,
                              char *);    // at "p", delete a 'size' byte hole
static char *text_hole_make(char *, int); // at "p", make a 'size' byte hole
static char *yank_delete(char *, char *, int,
                         int);      // yank text[] into register then delete
...
#if ENABLE_FEATURE_VI_SEARCH
static char *char_search(char *, const char *, int,
                         int); // search for pattern starting at p
static int mycmp(const char *, const char *,
                 int); // string cmp based in "ignorecase"
#endif
#if ENABLE_FEATURE_VI_COLON
static char *get_one_address(char *, int *); // get colon addr, if present
static char *get_address(char *, int *,
                         int *); // get two colon addrs, if present
...
void *xzalloc(size_t size) { return memset(xmalloc(size), 0, size); }
...
int bb_putchar(int ch) { return putc(ch, stdout); }
...
static void write1(const char *out) { fputs(out, stdout); }
  ...
  while (editing > 0) {
    ...
    if (crashme > 0) {
      if ((end - text) > 1) {
        ...
        crashme = 0;
        dot =
            string_insert(text, // insert the string
...
static char *get_address(char *p, int *b,
                         int *e) // get two colon addrs, if present
{
...
static void colon(char *buf) {
  ...
  *buf1 = '\0';
  ...
  if (i == 0) { // :123CR goto line #123
    ...
    li = count_lines(text, end - 1);
    status_line(
        "\"%s\"%s" USE_FEATURE_VI_READONLY("%s") " %dL, %dC", current_filename,
        (file_size(fn) < 0 ? " [New file]" : ""),
        USE_FEATURE_VI_READONLY(((readonly_mode) ? " [Readonly]" : ""), ) li,
        ch);
  ...
  } else if (strncasecmp(cmd, "features", i) ==
    ...
    status_line(
        "\"%s\"" USE_FEATURE_VI_READONLY("%s") " %dL, %dC", fn,
    ...
    *cursor = 0;
    ...
    cursor += printf(cursor, "tabstop=%d ", tabstop);
             ...
             0) { // substitute a pattern with a replacement pattern
    char *ls, *F, *R;
    int gflag;
    ...
    *R++ = '\0'; // terminate "find"
    ...
    status_line(BB_VER " " BB_BT);
             ...
             strncasecmp(cmd, "wn", i) == 0 || strncasecmp(cmd, "x", i) == 0) {
  ...
  vc3:;
    ...
    text_yank(q, r, YDreg);
    li = count_lines(q, r);
    status_line("Yank %d lines (%d chars) into [%c]", li, strlen(reg[YDreg]),
                what_reg());
#endif
  } else {
    // cmd unknown
...
static void dot_next(void) { dot = next_line(dot); }

static void dot_prev(void) { dot = prev_line(dot); }
...
{
  FILE *f = fopen("/tmp/vi_debug.log", "a");
  if (f) {
    fprintf(f, "char_insert input: c=%d ('%c'), erase_char=%d\n", c,
            (c >= 32 && c < 127) ? c : '.', erase_char);
    fclose(f);
  }
  if (c == 22) {               // Is this an ctrl-V?
    ...
    *p = c;
    ...
    // Delete the previous char, including '\n', so backspace can join lines.
    if (p > text) {
      p--;
...
//  open a hole in text[]
static char *text_hole_make(char *p,
...
static char *text_hole_delete(char *p,
                              char *q) // delete "p" through "q", inclusive
{
  char *src, *dest;
  int cnt, hole_size;
...
static void cookmode(void) { tcsetattr(0, TCSANOW, &term_orig); }
...
static int file_insert(const char *fn,
                       char *p USE_FEATURE_VI_READONLY(,
...
key_cmd_mode:
  ...
  case ':':                  // :- the colon mode commands
    ...
    if (strncasecmp(p, "quit", cnt) == 0 ||
      ...
      if (cnt < 0) {
        ...
        file_modified = 0;
        last_file_modified = -1;
        status_line("\"%s\" %dL, %dC", current_filename,
                    count_lines(text, end - 1), cnt);
        if (p[0] == 'x' || p[1] == 'q' || p[1] == 'n' || p[0] == 'X' ||
            p[1] == 'Q' || p[1] == 'N') {
          editing = 0;
        }
      }
  ...
  case 'O': // O- open a empty line above
    ...
    if (p[-1] == '\n') {
      dot_prev();
    case 'o': // o- open a empty line below; Yes, I know it is in the middle of
              // the "if (..."
      dot_end();
      dot = char_insert(dot, '\n');
    } else {
...
#endif
  {
    ...
    if (c1 != 27) {
      ...
      status_line("%s %d lines (%d chars) using [%c]", buf, cnt,
                  strlen(reg[YDreg]), what_reg());
#endif
      end_cmd_q(); // stop adding to q
    }
  } break;
...
static const char *const words[20] = {
    "this", "is",     "a",     "test",    "broadcast", "the",    "emergency",
    "of",   "system", "quick", "brown",   "fox",       "jumped", "over",
    "lazy", "dogs",   "back",  "January", "Febuary",   "March"};
static const char *const lines[20] = {
    ...
    "I have tickets available for the Blazers vs LA Clippers for Monday, "
    "Janurary 1 at 1:00pm.\n",
...
static void crash_test() {
  ...
  if (msg[0]) {
    printf("\n\n%d: \'%c\' %s\n\n\n%s[Hit return to continue]%s", totalcmds,
           last_input_char, msg, SOs, SOn);
    fflush(stdout);
  ...
  tim = time(NULL);
  if (tim >= (oldtim + 3)) {
    sprintf(status_buffer, "Tot=%d: M=%d N=%d I=%d D=%d Y=%d P=%d U=%d size=%d",
